...
public class MemRegisters {
     ...
     * Bits 6-8 are not used.
     * Sound Registers - controls the four GB Sound Channels (1 and 2 are
     * squarewave, 3 is waveform output, 4 is noise). Located between 0xFF10 and
     * 0xFF3f
     * Each Channel has 4-5 registers
     * C1: NR11, Sweep Register. NR12, Sound Length/Wave Pattern Duty. NR13,
     * Frequency Lo. NR14, Frequency Hi/Control
     * C2: NR21, Sound Length/Wave Pattern Duty. NR22, Envelope. NR23, Freq Lo.
     * NR24, Freq Hi/Control
     * C3: NR30, Sound On/Off. NR31, Sound Length. NR32, Select Output Level. NR33,
     * Freq Lo. NR34, Freq Hi/Control
     * C4: NR41, Sound Length. NR42, Envelope. NR43, Polynomial Counter. NR44,
     * Control
     * LCD Registers - LCD Registers control the graphics system and are located
     * between 0xFF40 and 0xFF4B. There are 4 types.
     ...
     * BGPD (Background Pallete Data): COntains color values for GB's Background
     * palette
     * BGP (Background Palette): Specifies which colors are used for Background
     * Palette
     * Window Registers
     * WY (Window Y): Specifies Y Coord of Window
     * WX (Window X): Specifies X Coord of Window
     * Object (Sprite) Registers
     * OAM (Object Attribute Memory): Contains information about the sprites that
     * are currently on display
     * OBJPD (Object Palette Data): This register contains the color values for the
     * GB's Object Palette
     * OBP0 (Object Palette 0): Specifies which colors are used for the first object
     * palette
     * OBP1 (Object Palette 1): Specifies which colors are used for the second
     * object palette
     * Timer registers - The timer registers control the Gameboy's timer system,
     * located form 0xFF04 ot 0xFF07
     * DIV (Divider Register): FF04, 8 bit read only that is incremented at a rate
     * of 16384 Hz, generating a clock signal for the timer system
     * TIMA (Timer Counter): FF05, 8 bit counter incremeented at a rate determined
     * by the value in the TAC register. When it overflows at 0xFF, generates and
     * interrupt
     * TMA (Timer Modulo): FF06, 8 bit register that sets the starting value of the
     * TIMA counter, setting TIMA to its value upon an overflow
     * TAC (Timer Control): FF07, 8 bit Register that controls the behavior of the
     * timer system
     ...
     * Bit 3: Timer stop
     * Bit 4-8: unused
     * Joypad Register - Detects a control input on the joypad for the game to
     * interact with, located at 0xFF00
     * Bit 6: indicates whether the register reflects the face buttons (0) or
     * doesn't (1)
     ...
     * There are serial i/o registers, but we might not implement them since link
     * cable emulation is not extremely important
     */
